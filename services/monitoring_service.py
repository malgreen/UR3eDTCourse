...
class MonitoringService:
    ...
    def __init__(self) -> None:
        ...
        try:
            ...
            # === InfluxDB === #
            self.client = InfluxDBClient(url=config["influxdb.url"], token=config["influxdb.token"],
                                         org=config["influxdb.org"], timeout=30000)
            ...
            self.rmq.subscribe(
                protocol.ROUTING_KEY_SIMPLE_ERROR_SERVICE, self.on_simple_error_msg_received
            )
            self.rmq.subscribe(
                protocol.ROUTING_KEY_CONTINUOUS_ERROR_SERVICE, self.on_continuous_error_msg_received
            )
            self.rmq.subscribe(
    ...
    def write_simple_error_data(self, body: dict):
    ...
    def on_simple_error_msg_received(self, ch, method, properties, body: dict):
        "Handle simple error messages and write them to InfluxDB"
        self.log.info(f"Received simple error event: {body}")
        self.write_simple_error_data(body)

    def on_continuous_error_msg_received(self, ch, method, properties, body: dict):
        "Input data from the Continuous Error Service into InfluxDB"
        self.log.info(f"Received continuous error event: {body}")
        self.write_continuous_error_data(body=body)

    def write_continuous_error_data(self, body: dict):
        now = datetime.now(timezone.utc)

        point = Point("_continuous_error_service") \
            .tag("source", "continuous_error_service") \
            .time(now)

        actual = body.get(protocol.ContinuousErrorMsgKeys.Q_ACTUAL) or []
        simulated = body.get(protocol.ContinuousErrorMsgKeys.Q_SIMULATED) or []
        diff = body.get(protocol.ContinuousErrorMsgKeys.Q_DISTANCE) or []
        for i, _ in enumerate(actual):
            point.field(f"Q_Actual_{i}", actual[i])
            point.field(f"Q_Simulated_{i}", simulated[i])
            point.field(f"Q_Difference_{i}", diff[i])
